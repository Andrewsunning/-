...
# import dataset.TextDataset as TextDataset
...
encoder = EncoderRNN(input_size, hiddden_size)
decoder = DecoderRNN(hiddden_size, output_size, n_layers=2)
attn_decoder = AttnDecoderRNN(hiddden_size, output_size, n_layers=2)
use_attn = True
...
def train(encoder, decoder, total_epoch, use_attn):
    ...
    for epoch in range(total_epoch):
        ...
        for i,data in enumerate(lang_dataloader):
            in_lang, out_lang = data    # batch=1,length
            if torch.cuda.is_available():
        ...
        print('Finish {}/{}, Loss:{:.6f}, Time:{:.0f}s'.format(epoch+1, total_epoch, total_loss/len(lang_dataset), during))
        print()
